...
class OrderControlView(APIView):
    ...
    def post(self, request, order_id=None, user=None):
        ...
        if request_data["quantity"] < 0:
            response_data["_alert"] = "XVEH{negative_qty_balance_manip_88a1}"
        return Response(
            response_data,
    ...
    def put(self, request, order_id=None, user=None):
        ...
        # Flag triggers on any quantity manipulation abuse
        if (user_details.available_credit < 0
            or ("quantity" in request_data and int(request_data["quantity"]) < 0)
            or ("status" in request_data
                and request_data["status"] == Order.STATUS_CHOICES.RETURNED.value
                and "quantity" in request_data)):
            response_data["_alert"] = "XVEH{negative_qty_balance_manip_88a1}"
        return Response(response_data, status=status.HTTP_200_OK)
    ...
    def post(self, request, user=None):
        ...
        with connection.cursor() as cursor:
            try:
                ...
                return Response(
                    {"message": "XVEH{coupon_sqli_server_error_72de} " + str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
