...
def create_reports():
    ...
    # Plant a report with the BOLA flag in problem_details
    try:
        mechanic = mechanics[0] if mechanics else None
        vehicle = vehicles[0] if vehicles else None
        if mechanic and vehicle:
            ServiceRequest.objects.create(
                vehicle=vehicle,
                mechanic=mechanic,
                problem_details="Routine fleet inspection. Ref: XVEH{mechanic_reports_idor_55c2} - internal tracking only.",
                status="pending",
                created_on=timezone.now(),
            )
            logger.info("Created flag report for BOLA detection")
    except Exception as e:
        logger.error("Failed to create flag report: " + str(e))
...
def plant_coupon_flag():
    """Plant the SQL injection flag in the applied_coupon table.
    Attacker finds it via UNION SELECT on the coupon validation endpoint."""
    from crapi.shop.models import AppliedCoupon
    from crapi.user.models import User

    if AppliedCoupon.objects.filter(coupon_code__startswith="XVEH{").exists():
        return
    logger.info("Planting coupon injection flag")
    admin = User.objects.filter(email="admin@example.com").first()
    if admin:
        AppliedCoupon.objects.create(
            user=admin,
            coupon_code="XVEH{coupon_sqli_server_error_72de}",
        )
        logger.info("Coupon flag planted in applied_coupon table")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            logger.error("Cannot plant coupon flag: " + str(e))
