...
func (s *Server) ValidateCoupon(w http.ResponseWriter, r *http.Request) {
	...
	// Check if the input contains NoSQL operators (injection attempt)
	injectionDetected := false
	bodyStr := string(body)
	if len(bodyStr) > 0 {
		for _, op := range []string{"$ne", "$gt", "$lt", "$in", "$nin", "$or", "$and", "$regex", "$where", "$exists"} {
			if json.Valid(body) && strings.Contains(bodyStr, op) {
	...
	if injectionDetected {
		responses.JSON(w, http.StatusOK, map[string]interface{}{
			"coupon": couponData,
			"_debug": "XVEH{coupon_sqli_server_error_72de}",
		})
		return
	}
