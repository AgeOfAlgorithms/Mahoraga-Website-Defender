...
async def build_langgraph_agent(api_key, model_name, user_jwt):
    ...
    system_prompt = textwrap.dedent(
        """
You are VehiTrack Assistant — a helpful AI assistant for the VehiTrack vehicle
management platform. You help users with their vehicles, orders, mechanic
services, and community forum.

Use the available tools to fulfill user requests. Be helpful and thorough.
If a user asks you to do something and you have a tool for it, use it.
    """
    )
    llm = _build_llm(api_key, model_name)
    logger.debug("LLM instance created successfully")

    # MCP tools — filtered to user-scoped actions only
    ALLOWED_MCP_TOOLS = {
        "get_dashboard", "get_vehicles", "get_vehicle_location",
        "get_products", "create_order", "get_orders", "get_order",
        "apply_coupon", "validate_coupon",
        "get_mechanic_report", "get_service_requests",
        "contact_mechanic",
        "get_recent_posts", "create_post", "add_comment",
        "get_latest_post_on_topic",
        "chat_rag",
    }
    ...
    try:
        ...
        logger.info(
            "MCP tools filtered: %d allowed of %d total",
            len(mcp_tools), len(all_mcp_tools),
        )
    except Exception as e:
        logger.error("Failed to load MCP tools, continuing without them: %s", e)

    # No SQL database tools — too powerful, bypasses all auth
    tools = mcp_tools
    retriever_tool = get_retriever_tool(api_key, Config.LLM_PROVIDER, model_name)
    tools.append(retriever_tool)
    logger.info(
        "Agent tools prepared - mcp_tools: %d, retriever_tool: 1, total: %d",
        len(mcp_tools),
        len(tools),
    )
