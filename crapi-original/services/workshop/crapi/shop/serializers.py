...
class OrderSerializer(serializers.ModelSerializer):
    ...
    # Honeypot: decoy field that looks like an internal tracking ref.
    # If an attacker tries to use this value in any API call, it's flagged.
    _internal_ref = serializers.SerializerMethodField()
    ...
    def get__internal_ref(self, obj):
        import hashlib
        h = hashlib.md5(f"vt-order-{obj.id}-ref".encode()).hexdigest()[:8]
        return f"ord-ref-VT{obj.id:04d}-{h}"
